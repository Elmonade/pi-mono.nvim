...
---@field range? pi.context.Range The operator range or visual selection range.
local Context = {}
Context.__index = Context

local ns_id = vim.api.nvim_create_namespace("PiContext")
...
function Context:render(prompt)
  local contexts = require("pi-mono.config").opts.contexts or {}
  ...
  for _, context_placeholder in ipairs(context_placeholders) do
    placeholders[context_placeholder] = {
      input = function()
        return { context_placeholder, "PiContextPlaceholder" }
      end,
      output = function()
        local value = contexts[context_placeholder](self)
        if value then
          return { value, "PiContextValue" }
        else
          return { context_placeholder, "PiContextPlaceholder" }
        end
      end,
...
function Context:diagnostics()
  ...
  for _, diagnostic in ipairs(diagnostics) do
    ...
    table.insert(diagnostic_strings, string.format(
      "- %s (%s): %s",
      location,
...
function Context:marks()
  ...
  for _, mark in ipairs(vim.fn.getmarklist()) do
    if mark.mark:match("^'[A-Z]$") then
      table.insert(marks, Context.format({
        buf = mark.pos[1],
